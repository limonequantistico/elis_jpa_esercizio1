...
public class BusinessLogic {
	...
	public BusinessLogic(BancaDao bancaDao, ContoCorrenteDao contoCorrenteDao, PersonaDao personaDao,
	...
	public void spostaFondi(String nome1, String cognome1, String nome2, String cognome2, Float importo) {
		...
		// Deve cercare su db due persone tramite nome e cognome, solo se le trove
		// entrambe deve decrementare
		// il saldo di tutti i conti della prima persona ed incrementare tutti quelli
		// della seconda in base
		// all'importo inserito, il tutto in modo transazionale

		List<Persona> p1 = personaDao.findByNomeAndCognome(nome1, cognome1);
		List<Persona> p2 = personaDao.findByNomeAndCognome(nome2, cognome2);

		if (p1.size() > 0 && p2.size() > 0) {

			for (ContoCorrente conto : p1.get(0).getConti()) {
				conto.setSaldo(conto.getSaldo() - importo);
			}

			for (ContoCorrente conto : p2.get(0).getConti()) {
				conto.setSaldo(conto.getSaldo() + importo);
			}
		}
	}
